...
@bot.tree.command(name="register", description="DBにあなたのDiscordIDを登録します")
async def register(interaction: discord.Interaction):

    discord_id = interaction.user.id  # 実行者のIDを取得
...
@bot.tree.command(name="dice", description="サイコロを振る（デフォルト6面）")
async def dice(interaction: discord.Interaction, sides: int = 6):
    if sides < 2:
        await interaction.response.send_message("⚠️ サイコロの面数は2以上を指定してください。", ephemeral=True)
        return
    result = random.randint(1, sides)
    await interaction.response.send_message(f"🎲 {sides}面サイコロの結果は… **{result}** です！")
...
async def on_guild_join(guild: discord.Guild):
    ...
    # ==============================
    # VersusCord カテゴリを取得 or 作成
    # ==============================
    category = discord.utils.get(guild.categories, name="VersusCord")

    if category is None:
        try:
            category = await guild.create_category("VersusCord")
            print(f"✅ VersusCordカテゴリを作成しました in {guild.name}")
        except discord.Forbidden:
            print("❌ カテゴリ作成権限がありません")
            return
    else:
        print(f"ℹ️ VersusCordカテゴリは既に存在します in {guild.name}")
    ...
    if channel is None:
        try:
            ...
            print("✅ versus-control チャンネルを作成しました")
        except discord.Forbidden:
    ...
    try:
        ...
        print("✅ MatchControlView / RegisterButtonView を送信しました")
